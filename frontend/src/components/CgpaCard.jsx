export default function CgpaCard({ profile }) {
  ...
  // Show "--" if no semesters or cgpa is 0 (new user)
  const hasSems = (profile?.semesters?.length ?? 0) > 0;
  const cgpa    = hasSems && typeof rawCgpa === "number" ? rawCgpa : null;
    ...
    <section className="glass-card-rich flex flex-col gap-5">
      ...
      <div className="relative h-2 w-full overflow-hidden rounded-full"
        ...
        <div className="absolute left-0 top-0 h-full rounded-full transition-all duration-700"
          style={{
            width: `${pct}%`,
            background: grade ? `linear-gradient(90deg, ${grade.color}88, ${grade.color})` : "rgba(255,255,255,0.12)",
          }} />
      </div>
      ...
      <div className="grid grid-cols-3 gap-3">
        <div className="stat-chip">
          ...
          <p className="mt-1 text-lg font-bold text-white" style={{ fontFamily: "Syne,sans-serif" }}>
            {semCount > 0 ? semCount : "--"}
          </p>
        </div>
        <div className="stat-chip">
